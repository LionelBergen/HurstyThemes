...
function disable_script() {
    ...
    if [[ ${ifexist} > "0" ]]
    then
        perl -pi -w -e 's\/$retropie_scripts_dir/themerandom.sh/#\/$retropie_scripts_dir/themerandom.sh/g;' /opt/retropie/configs/all/autostart.sh
    fi
    sleep 2
}
...
function enable_script() {
    ...
    if [[ ${ifexist} > "0" ]]
    then
        perl -pi -w -e 's/#\/$retropie_scripts_dir/themerandom.sh/\/home\/pi\/scripts\/themerandom.sh/g;' /opt/retropie/configs/all/autostart.sh
    else
        cp /opt/retropie/configs/all/autostart.sh /opt/retropie/configs/all/autostart.sh.bkp
        echo "$retropie_scripts_dir/themerandom.sh" > /tmp/autostart.sh
    ...
    while true; do
        ...
        case "$choice" in
            U)  #update install script to get new theme listings
                cd "$retropie_menu_dir" 
                mv "hurstythemes.sh" "hurstythemes.sh.bkp" 
                wget "https://raw.githubusercontent.com/LionelBergen/HurstyThemes/master/hurstythemes.sh" 
                if [[ -f "$retropie_menu_dir/hurstythemes.sh" ]]; then
                  echo "$retropie_menu_dir/hurstythemes.sh" > /tmp/errorchecking
                 else
